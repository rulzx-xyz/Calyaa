...
const HeartIcon: React.FC<{ animationDelay: string, rotation: number }> = ({ animationDelay, rotation }) => (
    <svg 
        className="fill-transparent opacity-0"
        style={{ animation: `love 3s forwards infinite ${animationDelay}`, transform: `scale(0.5) rotate(${rotation}deg)` }}
        viewBox="0 0 512 512"
    >
      ...
      {bubblesData.map((bubble, index) => (
        <div
          ...
          style={{
            ...
            height: bubble.size,
            width: bubble.size,
            animation: `love-burst 3s infinite ${bubble.delay}`,
            transform: `scale(0)`,
          }}
        >
          <HeartIcon animationDelay={bubble.delay} rotation={bubble.rotate}/>
        </div>
      ))}
    </div>
  );
